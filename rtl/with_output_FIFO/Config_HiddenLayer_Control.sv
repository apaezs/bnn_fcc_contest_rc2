...
module Config_HiddenLayer_Control #(
    ...
    output logic                t_cfg_valid,
    output logic [BANK_W-1:0]   t_cfg_bank,
    output logic [T_ADDR_W-1:0] t_cfg_addr,
    output logic [TW-1:0]       t_cfg_data,
    output logic cfg_done
);

    localparam logic MSG_TYPE_WEIGHTS = 1'b0;
    ...
    logic [TCOUNT_W-1:0]           t_word_count_r;

    logic                          cfg_done_r;
    logic                          have_weights_r;
    logic                          have_thresholds_r;

    logic                          w_start_r, w_start_next;
    logic                          t_start_r, t_start_next;
    logic                          t_active_r;
    logic                          w_active_r;
    logic                          w_fill_active_r;

    logic                          w_done_r, w_done_next;
    logic                          t_done_r, t_done_next;
    ...
    logic [31:0]                   t_word_r;
    logic [1:0]                    t_byte_count_r;
    logic [BANK_W-1:0]             t_bank_idx_r;
    logic [T_ADDR_W-1:0]           t_addr_idx_r;
    ...
    logic                          t_issue_valid_r;
    logic [BANK_W-1:0]             t_issue_bank_r;
    logic [T_ADDR_W-1:0]           t_issue_addr_r;
    logic [TW-1:0]                 t_issue_data_r;
    ...
    logic t_last_byte_fire;
    logic t_last_word_fire;
    logic t_bank_wrap_fire;
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (w_done_r && have_thresholds_r)
                cfg_done_r <= 1'b1;
            else if (t_done_r && have_weights_r)
                cfg_done_r <= 1'b1;
            if (w_adv_last_word_r)
                have_weights_r <= 1'b1;
            if (t_last_word_fire)
                have_thresholds_r <= 1'b1;
    ...
    assign run_launch_fire       = msg_sink_valid_r && !(w_active_r || t_active_r);
    ...
    assign t_last_byte_fire = payload_sink_valid_r && (t_byte_count_r == 2'd3);
    assign t_last_word_fire = t_last_byte_fire && ((t_word_count_r + 1'b1) == TOTAL_T_WORDS);
    assign t_bank_wrap_fire = (t_bank_idx_r == PN-1);

    always_comb begin
        w_start_next = run_launch_fire && (msg_sink_type_r == MSG_TYPE_WEIGHTS);
        t_start_next = run_launch_fire && (msg_sink_type_r != MSG_TYPE_WEIGHTS);
    end
    ...
    always_comb begin
        ...
        if (t_active_r && payload_sink_valid_r)
            payload_sink_valid_next = 1'b0;
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            t_start_r <= t_start_next;
            w_done_r  <= w_done_next;
            t_done_r  <= t_done_next;
        end
    end
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (w_launch_fire) w_launch_byte_r <= payload_sink_data_r;
        end
    end
    ...
    always_ff @(posedge clk) begin
        if (w_start_r)
            w_bank_idx_r <= '0;
        if (w_prep_valid_r && w_adv_last_beat_r)
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            t_clear_bank_r <= t_last_byte_fire && t_bank_wrap_fire;
            t_bump_bank_r  <= t_last_byte_fire && !t_bank_wrap_fire;
            t_bump_addr_r  <= t_last_byte_fire && t_bank_wrap_fire;
        end
    end
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (t_start_r) begin
                ...
                if (t_clear_bank_r)
                    t_bank_idx_r <= '0;
                else if (t_bump_bank_r)
                    t_bank_idx_r <= t_bank_idx_r + 1'b1;

                if (t_bump_addr_r)
                    t_addr_idx_r <= t_addr_idx_r + 1'b1;

                if (t_last_byte_fire) begin
                    t_issue_valid_r <= 1'b1;
                    t_word_count_r  <= t_word_count_r + 1'b1;
                    t_byte_count_r  <= 2'd0;
                end else if (payload_sink_valid_r) begin
                    t_byte_count_r <= t_byte_count_r + 1'b1;
                end

                case (t_byte_count_r)
                    2'd0: t_word_r[7:0]   <= payload_sink_data_r;
                    2'd1: t_word_r[15:8]  <= payload_sink_data_r;
                    2'd2: t_word_r[23:16] <= payload_sink_data_r;
                endcase

                t_issue_bank_r <= t_bank_idx_r;
                t_issue_addr_r <= t_addr_idx_r;
                t_issue_data_r <= {payload_sink_data_r, t_word_r[23:0]};
            end
        end
    end
    ...
    always_comb begin
        ...
        t_cfg_valid   = t_active_r ? t_issue_valid_r : '0;
        t_cfg_bank    = t_issue_bank_r;
        t_cfg_addr    = t_issue_addr_r;
        t_cfg_data    = t_issue_data_r;
