...
module NP_MEM_WithThreshold #(
    ...
    parameter int T_ADDR_W           = 8,
    parameter int LAT                = 4,
    localparam int ACC_W             = $clog2(TOTAL_BITS_NEURON+1),
    localparam int TH_CTRL_DELAY     = LAT - 4
)(
    input  logic clk,
    input  logic rst,
    ...
    input  logic [T_ADDR_W-1:0] t_ram_b_addr,

    input  logic                w_ram_ren_a,
    input  logic                w_ram_wen_a,
    input  logic [W_ADDR_W-1:0] w_ram_a_addr,
    input  logic [PW-1:0]       w_ram_a_data,
    input  logic                t_ram_ren_a,
    input  logic                t_ram_wen_a,
    input  logic [T_ADDR_W-1:0] t_ram_a_addr,
    input  logic [TW-1:0]       t_ram_a_data,

    output logic [TW-1:0]       t_ram_a_rdata,
  ...
  logic                t_ram_ren_b_r;
  logic [T_ADDR_W-1:0] t_ram_b_addr_r;
  ...
  logic                t_ram_ren_a_r;
  logic                t_ram_wen_a_r;
  logic [T_ADDR_W-1:0] t_ram_a_addr_r;
  logic [TW-1:0]       t_ram_a_data_r;
  logic                t_ram_ren_b_d;
  logic [T_ADDR_W-1:0] t_ram_b_addr_d;
  logic [PW-1:0]       w_ram_b_rdata;
  logic [TW-1:0]       t_ram_b_rdata;
  ...
  logic                t_mem_ren;
  logic                t_mem_wen;
  logic [T_ADDR_W-1:0] t_mem_addr;
  logic [TW-1:0]       t_mem_wdata;
  logic [TW-1:0]       t_mem_rdata;
  ...
  always_ff @(posedge clk) begin
    ...
    t_ram_ren_b_r  <= t_ram_ren_b;
    t_ram_b_addr_r <= t_ram_b_addr;
    last_in_r      <= last_in;
  end
  ...
  always_ff @(posedge clk) begin
    ...
    t_ram_ren_a_r  <= t_ram_ren_a;
    t_ram_wen_a_r  <= t_ram_wen_a;
    t_ram_a_addr_r <= t_ram_a_addr;
    t_ram_a_data_r <= t_ram_a_data;
  end
  ...
  assign t_mem_wen   = t_ram_wen_a_r;
  assign t_mem_ren   = t_ram_ren_b_d;
  assign t_mem_addr  = t_ram_wen_a_r ? t_ram_a_addr_r : t_ram_b_addr_d;
  assign t_mem_wdata = t_ram_a_data_r;
  ...
  assign t_ram_a_rdata = t_mem_rdata;
  assign t_ram_b_rdata = t_mem_rdata;
  ...
  NP_UNIT_WithThreshold #(
    ...
    .x             (x_in_np2),
    .w             (w_ram_b_rdata),
    .threshold     (t_ram_b_rdata[ACC_W-1:0]),
